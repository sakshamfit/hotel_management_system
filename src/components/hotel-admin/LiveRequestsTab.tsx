...
export const LiveRequestsTab: React.FC<Props> = ({ hotel }) => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    switch (s) {
      case 'PENDING':
      case 'NEW':
        return 'bg-accent-tint text-[#0066cc] border-accent-soft animate-pulse';
      case 'ACCEPTED':
      case 'IN_PROGRESS':
      case 'PREPARING':
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="bg-white border border-hairline rounded-xl p-12 text-center">
          ...
          <p className="text-xs text-ink-mute mt-3">Listening for incoming guest requests...</p>
        </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 text-xs text-ink-mute">
                      ...
                      <span className="font-semibold text-ink">{order.guestName || 'Guest'}</span>
                    </div>
                    {order.type && (
                  ...
                  {order.items && order.items.length > 0 && (
                    <div className="bg-canvas-soft border border-hairline p-3 rounded-lg text-xs space-y-1.5">
                      <div className="text-[10px] font-bold text-ink-mute uppercase">Order Items:</div>
                      ...
                      {order.totalAmount > 0 && (
                        <div className="flex items-center justify-between font-bold text-xs pt-1 border-t border-hairline">
                          <span>Total:</span>
                  ...
                  {order.instructions && (
                    <p className="text-xs text-ink-mute italic bg-canvas-soft/60 p-2.5 rounded-xl border border-hairline">
                ...
                <div className="mt-4 pt-3 border-t border-hairline flex items-center gap-2">
                  {status === 'PENDING' || status === 'NEW' ? (
                    <button
                      ...
                      className="flex-1 py-2 px-3 rounded-lg bg-[#0066cc] hover:bg-[#004fa3] text-white text-xs font-bold shadow-sm transition-colors"
                    >
                      Accept & Prepare
                    </button>
                    ...
                    <button
                      ...
                      disabled={updatingId === order.id}
                      className="flex-1 py-2 px-3 rounded-full bg-success-mid hover:bg-success-deep text-white text-xs font-bold shadow-sm transition-colors"
                    >
                      Mark Completed
                    </button>
                  ) : (
                    <div className="text-xs text-success-mid font-semibold flex items-center gap-1">
                  ...
                  {status !== 'COMPLETED' && status !== 'CANCELLED' && (
                    <button
                      ...
                      disabled={updatingId === order.id}
                      className="p-2 rounded-full hover:bg-accent-tint text-ink-mute hover:text-[#0066cc] border border-hairline hover:border-accent-soft transition-colors"
