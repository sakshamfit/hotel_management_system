...
export const HotelAdminLayout: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-canvas text-ink flex items-center justify-center p-6">
        <div className="card-feature-light p-8 max-w-md text-center space-y-4 elev-1">
          <div className="w-14 h-14 rounded-xl bg-accent-tint border border-accent-soft flex items-center justify-center text-primary mx-auto">
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink flex flex-col md:flex-row">
      {/* Sidebar — near-black canvas (the dark register); one blue accent for the active tab */}
      <aside className="w-full md:w-64 bg-surface-tile-1 text-on-primary md:border-r border-hairline-dark flex flex-col justify-between shrink-0">
        <div className="p-4 space-y-5">
          {/* Hotel identity */}
          <div className="bg-surface-tile-2 border border-hairline-dark p-3 rounded-xl flex items-center gap-3">
            {hotel.branding?.logoUrl ? (
              <img
                src={hotel.branding.logoUrl}
                alt={hotel.name}
                className="w-10 h-10 rounded-lg object-cover border border-hairline-dark bg-surface-tile-1"
              />
            ) : (
              <div className="w-10 h-10 rounded-lg bg-primary text-on-primary flex items-center justify-center font-bold border border-hairline-dark">
                {hotel.name.charAt(0)}
              </div>
            )}
            <div className="overflow-hidden">
              <h3 className="text-xs font-bold text-on-primary truncate leading-tight">{hotel.name}</h3>
              <div className="text-[10px] text-primary-on-dark font-mono font-semibold mt-0.5">{hotel.hotelCode}</div>
            </div>
          </div>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-lg text-xs transition-colors ${
                    isActive
                      ...
                      : 'text-on-dark-mute hover:text-on-primary hover:bg-surface-tile-2 font-medium'
                  }`}
                >
                    ...
                    <span
                      className={`text-[9px] uppercase font-bold px-2 py-0.5 rounded-full ${
                        isActive
                          ...
                          : 'bg-surface-tile-2 text-primary-on-dark border border-hairline-dark'
                      }`}
                    >
                      {tab.badge}
                    </span>
        ...
        <div className="p-4 border-t border-hairline-dark text-[11px] bg-surface-tile-3 space-y-1">
          <div className="flex items-center gap-1.5 text-success-tint font-semibold">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto bg-canvas">
        ...
        {activeTab === 'kitchen_kds' && (
          <PinGate hotel={hotel} department="KITCHEN" label="Kitchen">
            <KitchenDisplayTab hotel={hotel} />
          </PinGate>
        )}
        {activeTab === 'housekeeping' && (
          <PinGate hotel={hotel} department="HOUSEKEEPING" label="Housekeeping">
            <HousekeepingTab hotel={hotel} />
          </PinGate>
        )}
        {activeTab === 'rooms_qr' && <RoomsAndQrTab hotel={hotel} />}
        {activeTab === 'checkin' && (
          <PinGate hotel={hotel} department="RECEPTION" label="Reception">
            <GuestCheckinTab hotel={hotel} />
          </PinGate>
        )}
